...
#imdb_basics_crew <- imdb_basics_crew %>% rename(directors = nconst)
#print(imdb_basics_crew)
#Merge the dataframe that was created above with the imdb_name_basics data
imdb_basics_crew_name <- left_join(imdb_basics_crew, imdb_name_basics, by= c("directors" = "nconst"))
...
write.csv(filtered_data, file = "first_output.csv")
###second code

openingweekend_unique <- read.csv("openingweekend_unique.csv")

merged_dataset <- openingweekend_unique %>% left_join(imdb_basics_crew_name, by = c("name" = "primaryTitle"))
